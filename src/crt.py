...
def retro_filter(input_image: str | np.ndarray,
    ...
    y_coords, x_coords = np.meshgrid(np.arange(screenheight), np.arange(screenwidth), indexing='ij')

    #Create uv map of the coordinates for easier calculations.
    uv = np.stack((y_coords / (screenheight - 1), x_coords / (screenwidth - 1)), axis=-1)


    uv = uv * 2 - 1                 # Centers the uv map so it spans from -1 to 1.
    offset = uv / curvature         # Creates curvature map. Values in the center are close to zero and curve less than values at the borders which are closer to 1 or -1
    uv += uv * offset * offset      # Adds curved uv coordinates to the uv map.
    inverted_uv = 1 - np.abs(uv)    # Gets inverted uv coordinates for later use in the vignette
    ...
    vignette_vector = np.array([vignette_width / screenheight, vignette_width / screenwidth])
    ...
    new_image[..., 0] = np.clip(image[y, x, 0] * sin_vals * vignette[..., 0] * vignette[..., 1], 0, 1)
    new_image[..., 1] = np.clip(image[y, x, 1] * cos_vals * vignette[..., 0] * vignette[..., 1], 0, 1)
    new_image[..., 2] = np.clip(image[y, x, 2] * sin_vals * vignette[..., 0] * vignette[..., 1], 0, 1)
    return new_image
